        ...
        <div className="ideabox_ideacontent">
          ...
          <div className="item_blog_like_a text-right mb-2">
            ...
            {ideaDetail.ideaLikes.length > 0 &&
              <span className="text-right i_like_count ideabox_contentt_size" style={{ marginLeft: "5px" }}>{ideaDetail.ideaLikes.length} {ideaDetail.ideaLikes.length === 1 ? " Like" : " Likes"}</span>
            }
          </div>
                        ...
                        {cmtData.ideaCommentAttach.map((atthData, atthIndex) => {
                          return (
                            <div className="eep_command_attachements_inner_content" key={"cmdAtthList_"+atthIndex}>
                              <a href={atthData.docByte?.image} target="_thapa" download={atthData.ideaAttachmentsFileName}>
                                <img src={fileTypeAndImgSrcArray[atthData.contentType] ? fileTypeAndImgSrcArray[atthData.contentType] :  fileTypeAndImgSrcArray['default']} className="image-circle c1 attach_img_sm" alt="icon" title={atthData.ideaAttachmentsFileName} />
                              </a>
                            </div>
                          )
                        })}
