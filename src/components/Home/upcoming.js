                ...
                {dashboardDetails && dashboardDetails.upcomings && dashboardDetails.upcomings.map((item, index) => {
                    return (
                        <div className="mytasks_list_div text-left" key={"myTask_" + index}>
                            <div className="">
                                <Link to={{
                                    pathname: "ecardIndex", state: {
                                        activeTab: 'CardsTab', active: item?.active,
                                        isDashbaordData: {
                                            "value": item?.userId,
                                            "label": item?.fullName + " - " + item?.role
                                        }
                                    }
                                }} className="c-2c2c2c a_hover_txt_deco_none d_mytasks_list" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
