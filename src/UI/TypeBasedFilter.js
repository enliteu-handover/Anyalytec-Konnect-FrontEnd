...
const TypeBasedFilter = (props) => {
	...
	const [filterEndDate, setFilterEndDate] = useState(formatFilterDate(new Date()));
	...
	const [currentFourMonth, setCurrentFourMonth] = useState(Math.floor((new Date().getMonth() + 4) / 4));
	const [currentHalfYear, setCurrentHalfYear] = useState(Math.floor((new Date().getMonth() + 6) / 6));

	const monthArr = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
	const [filterDispValue, setFilterDispValue] = useState(monthArr[initCurrentMonth - 1] + ", " + initFullYear);
	...
	const submitFilterHandler = () => {
		...
		if (filterSettings.isMonth) {
			...
			obj.quarter = quarterArr.filter(res => res.quarterNo === currentQuarter)[0].value;
			obj.year = fullYear;
			updateFilterDispValue(quarterArr.filter(res => res.quarterNo === currentQuarter)[0].label + "," + fullYear);
		} else if (filterSettings.isFourMonth) {
			obj.fourmonths = fourMonths.filter(res => res.fourMonthNo === currentFourMonth)[0].value;
			obj.year = fullYear;
			updateFilterDispValue(fourMonths.filter(res => res.fourMonthNo === currentFourMonth)[0].label + "," + fullYear);
		} else if (filterSettings.isHalfYear) {
			obj.halfyear = halfYear.filter(res => res.halfYearNo === currentHalfYear)[0].value;
			obj.year = fullYear;
			updateFilterDispValue(halfYear.filter(res => res.halfYearNo === currentHalfYear)[0].label + "," + fullYear);
			...
			updateFilterDispValue(formatDate(filterDate) + " to " + formatDate(filterEndDate));
	...
	return (
		<div className="eep-options-div ml-auto my-auto">
			<div className="d-inline-block pr-4 eep_select_div d_filterby_div" id="d_filterby_div">
				<button type="button" className="btn d_filterby d-inline-flex align-items-center" onClick={() => setShowFilter(!showFilter)}>
				...
				<div className={`eep-filterOptions ${showFilter ? "d_filterOptionShow" : ""}`} style={{zIndex:99}}>
					...
					{filterSettings.isDate &&
						<div className={`filter-list-div filter-list-date filter-list-default`}>
							<DatePicker
								selected={startDate}
					...
					{filterSettings.isbetweenDates &&
						<React.Fragment>
							<label> From : </label>
							<div className={`filter-list-div filter-list-date filter-list-default mb-2`}>
							...
							<div className={`filter-list-div filter-list-date filter-list-default mb-2`}>
								<label> To : </label>
								<DatePicker selected={endDate} className="date-picker"
									calendarClassName="eep-date-picker"
					...
					{filterSettings.isMonth &&
						<div className={`filter-list-div filter-list-month filter-list-default`}>
							<ul className="d_filterOption d_filterMonth">
								{monthArr.map((res, index) => {
									return <li className="d_filterVal" key={index + 1}>
										<button type="button" className={`val ${(currentMonth === index + 1) ? 'selected' : ''}`} value={index + 1} onClick={() => selectMonthHandler(index + 1)}>{res}</button>
									</li>
								})}
							</ul>
						</div>
					...
					{filterSettings.isQuarter &&
						<div className={`filter-list-div filter-list-quarter filter-list-default`}>
							<ul className="d_filterOption d_filterQuarter">
								{quarterArr.map((res) => {
									return <li className="d_filterVal" key={res.value} >
										<button type="button" className={`val ${(currentQuarter === res.quarterNo) ? 'selected' : ''}`} value={res.value} onClick={() => selectQuarterHandler(res)}>{res.label}</button>
									</li>
								})}
							</ul>
						</div>
					...
					{filterSettings.isFourMonth &&
						<div className={`filter-list-div filter-list-four-months filter-list-default`}>
							<ul className="d_filterOption d_filterTerm">
								{fourMonths.map(res => {
									return <li className="d_filterVal" key={res.value}>
										<button type="button" className={`val ${(currentFourMonth === res.fourMonthNo) ? 'selected' : ''}`} value={res.value} onClick={() => selectFourMonthHandler(res)}>{res.label}</button>
									</li>
								})}
							</ul>
						</div>
					...
					{filterSettings.isHalfYear &&
						<div className={`filter-list-div filter-list-half filter-list-default`}>
							<ul className="d_filterOption d_filterHalf">
								{halfYear.map(res => {
									return <li className="d_filterVal" key={res.value}>
										<button type="button" className={`val ${(currentHalfYear === res.halfYearNo) ? 'selected' : ''}`} value={res.value} onClick={() => selectHalfYearHandler(res)}>{res.label}</button>
									</li>
								})}
							</ul>
						</div>
					...
					{filterSettings.isYear &&
						<div className={`filter-list-year mb-3`}>
							<div className="filter-year-prev filter-nav useserSelect" dangerouslySetInnerHTML={{ __html: svgIcons && svgIcons.LesserArrow }} onClick={() => yearChange(false)}>
							</div>
							<div className="filter-year-input">
								<input type="text" className="year_val font-helvetica-m" name="year_val" value={fullYear} onChange={(e) => yearFilter(e)} />
							</div>
							<div className="filter-year-next filter-nav useserSelect" dangerouslySetInnerHTML={{ __html: svgIcons && svgIcons.GreaterArrow }} onClick={() => yearChange(true)}>
							</div>
						</div>
					}
					<div className="filter-list-action useserSelect">
						<button type="button" className="eep-btn eep-btn-cancel eep-btn-nofocus eep-btn-xsml d_filterCancel" onClick={() => setShowFilter(false)}>Cancel</button>
						<button type="button" className="eep-btn eep-btn-success eep-btn-nofocus eep-btn-xsml ml-2 d_filterAction" onClick={submitFilterHandler}>Ok</button>
					</div>
				</div>
			</div>
		</div >
