...
const SortList = (props) => {
  ...
  const { readAllCommunicationsFromList, communicationPostLists, dateReceivedOrder, isFeed } = props;
      ...
      <div className="col-12 bg-white">
        <div className="filter bg-white">
          <div className="arrow_div" onClick={sortDateReceived}>
            {isFeed ?
              isDateReceived ? <span className="c1 i_position">Newest
                {!isDateReceived &&
                ...
                }</span> : <span className="c1 i_position">Oldest
                {!isDateReceived &&
                  <i className="fa fa-angle-up arrow_postion" style={{ fontSize: "15px" }}></i>
                }
                {isDateReceived &&
